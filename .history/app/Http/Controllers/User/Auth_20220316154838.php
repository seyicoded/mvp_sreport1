...
class Auth extends Controller
{
    ...
    public function login_in(Request $request){
        ...
        // check if user exist
        $dt = DB::select("SELECT * FROM sample_users WHERE email = ? AND password = ?", [$email, $password]);
        if(count($dt) != 0){
            setcookie(sha1('is_user_signed_in_kef_investment'),sha1('user_signed_in'), (time() + (86400 * 365) ),"/");
            setcookie(sha1('kef_user_id'),base64_encode('1'), (time() + (86400 * 365) ),"/");
            setcookie(sha1('kef_user_name'),base64_encode('1'), (time() + (86400 * 365) ),"/");
            return redirect($redirect_url);
        }else{
            return view('signin')->with('redirect_url', $redirect_url)->with('error', 'Account not found');
        }
    }
}
